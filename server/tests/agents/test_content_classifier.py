...
class TestContentClassifier:
    ...
    async def test_process_returns_expected_schema(self, content_classifier):
        ...
        expected_result = {"type": "article", "usefulness": "high", "relevance": 0.85}

        with patch.object(
            content_classifier, "execute", AsyncMock(return_value=expected_result)
        ):
            # Act
    ...
    async def test_process_with_empty_content(self, content_classifier):
        # Arrange
        content = ""
        expected_result = {"type": "unknown", "usefulness": "low", "relevance": 0.0}
