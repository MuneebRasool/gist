...
class TestPersonalitySummarizer:
    ...
    def sample_onboarding_data(self):
        ...
        data = {
            ...
            "email_ratings": [
                {"email_id": "123", "rating": 4, "reason": "Important project information"},
    ...
    @pytest.mark.asyncio
    async def test_process_onboarding_returns_string(self, personality_summarizer, sample_onboarding_data):
        # Arrange
        expected_result = "User is a Software Engineer in Technology who prefers collaborative work."
        
        with patch.object(personality_summarizer, 'execute', AsyncMock(return_value=expected_result)):
            # Act
            result = await personality_summarizer.process_onboarding(sample_onboarding_data)
    ...
    @pytest.mark.asyncio
    async def test_process_onboarding_handles_exception(self, personality_summarizer, sample_onboarding_data):
        # Arrange
        with patch.object(personality_summarizer, 'execute', AsyncMock(side_effect=Exception("Test error"))):
            # Act & Assert
