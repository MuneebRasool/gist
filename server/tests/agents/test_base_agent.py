...
class TestBaseAgent:
    ...
    async def test_execute_json_response(self, base_agent, mock_openai_client):
        ...
        json_response = (
            '{"name": "Assistant", "type": "AI", "capabilities": ["text", "code"]}'
        )
        expected_response = {
            "name": "Assistant",
            "type": "AI",
        ...
        result = await base_agent.execute(
            system_prompt, user_input, response_format="json"
        )
    ...
    async def test_execute_with_tool_calls(self, base_agent, mock_openai_client):
        ...
        with patch(
            "src.agents.base_agent.get_task_deadline",
            return_value={"deadline": "tomorrow at 5 PM"},
        ):
            # Act
            result = await base_agent.execute(
                system_prompt, user_input, tool_schemas=tool_schemas
            )
    ...
    @pytest.mark.asyncio
    async def test_execute_handles_api_error_json_format(
        self, base_agent, mock_openai_client
    ):
        # Arrange
        system_prompt = "You are a helpful assistant."
        user_input = "Hello, how are you?"
