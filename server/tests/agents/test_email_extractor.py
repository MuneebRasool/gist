...
class TestEmailExtractorAgent:
    ...
    def mock_domain_inference_agent(self):
        with patch("src.agents.email_extractor.DomainInferenceAgent") as mock_agent:
            ...
            domain_agent_instance.infer_domain = AsyncMock(
                return_value={
    ...
    def mock_email_scorer_agent(self):
        with patch("src.agents.email_extractor.EmailScorerAgent") as mock_agent:
            ...
            email_scorer_instance.score_email = AsyncMock(
                return_value={
                    "email_id": "test_id_1",
                    "score": 0.85,
    ...
    @pytest.fixture
    def email_extractor_agent(
        self, mock_file_utils, mock_domain_inference_agent, mock_email_scorer_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_extract_relevant_email_returns_expected_schema(
        self, email_extractor_agent, sample_emails
    ):
        # Arrange
        ...
        with patch.object(
            email_extractor_agent, "execute", AsyncMock(return_value=expected_result)
        ):
            # Act
            result = await email_extractor_agent.extract_relevant_email(
                sample_emails, user_domain
            )
    ...
    @pytest.mark.asyncio
    async def test_score_emails_by_domain_returns_expected_schema(
        self, email_extractor_agent, sample_emails
    ):
        # Act
        result = await email_extractor_agent.score_emails_by_domain(
            sample_emails, "user@example.com"
        )
    ...
    @pytest.mark.asyncio
    async def test_process_email_batches_returns_expected_schema(
        self, email_extractor_agent, sample_emails
    ):
        # Arrange
        user_domain = "example.com"
        ...
        # Act
        result = await email_extractor_agent.process_email_batches(
            sample_emails, user_domain
        )
