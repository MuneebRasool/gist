...
class TestContentSummarizer:
    ...
    def test_process(self, content_summarizer):
        # Arrange
        mock_emails = ["Email 1 content here", "Email 2 with some other content"]
        expected_result = {"summary": "This is a summary of the emails"}

        with patch.object(
            content_summarizer, "execute", MagicMock(return_value=expected_result)
        ):
            # Act
    ...
    async def test_process_content_returns_expected_schema(self, content_summarizer):
        ...
        with patch.object(
            content_summarizer, "execute", AsyncMock(return_value=expected_result)
        ):
            # Act
    ...
    @pytest.mark.asyncio
    async def test_process_content_adds_summary_field_if_missing(
        ...
        with patch.object(
            content_summarizer, "execute", AsyncMock(return_value=api_return_value)
        ):
            # Act
            result = await content_summarizer.process_content(content)
