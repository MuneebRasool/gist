...
export const useDrawerTasksStore = create<DrawerTasksState>((set) => ({
	tasks: [],
	isLoading: false,
	...
	fetchTasks: async (userId) => {
		set({ isLoading: true });
		try {
			const response = await TasksService.getUserDrawerTasks(userId);
			if (response.data) {
				set({ tasks: response.data });
			}
		} catch (error) {
			console.error('Error fetching drawer tasks:', error);
		} finally {
			set({ isLoading: false });
		}
	},
