...
const SortableTaskItem = ({ task, isDragging, isCompleted, onToggleComplete }: SortableTaskItemProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className="py-4 px-6">
      <div className="flex items-start gap-4">
        <button
          ...
          className="mt-1 w-4 h-4 rounded border border-gray-300 flex items-center justify-center shrink-0"
        >
          {isCompleted && (
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`text-lg font-medium text-gray-900 ${isCompleted ? 'line-through text-gray-400' : ''}`}>
            {task.task}
          </h3>
          {task.task && (
            <p className={`mt-1 text-sm text-gray-500 ${isCompleted ? 'line-through text-gray-400' : ''}`}>
              {task.task}
            </p>
          )}
        </div>
...
const SortableTaskList = ({ tasks, emptyMessage, onTasksReordered }: SortableTaskListProps) => {
  ...
  if (sortedTasks.length === 0) {
    return (
      <div className='flex flex-col items-center gap-6 rounded-3xl bg-white/30 backdrop-blur-md py-12'>
    ...
    <div className="w-full max-w-7xl h-full">
      ...
      >
        <SortableContext items={sortedTasks.map(task => task.task_id)} strategy={verticalListSortingStrategy}>
          <div className='rounded-3xl bg-white/30 backdrop-blur-md overflow-hidden'>
            {sortedTasks.map((task, index) => (
              <React.Fragment key={task.task_id}>
                ...
                {index < sortedTasks.length - 1 && (
                  <div className="h-px bg-black/20 mx-6" />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        <DragOverlay adjustScale={true}>
          {activeTask ? (
            <div className="w-full bg-white/40 backdrop-blur-md rounded-3xl shadow-lg">
